...
@st.cache_resource
def load_model():
    model_root = os.path.join(os.getcwd(), ".insightface")
...
def process_recorded_video(model, input_video_path, output_video_path, threshold, frame_stride=1):
    ...
    while True:
        ...
        if frame is None or not isinstance(frame, np.ndarray) or frame.size == 0:
            ...
            frame_no += 1
            writer.write(np.zeros((height, width, 3), dtype=np.uint8))  # keep timeline consistent
            continue
        ...
        if total_frames > 0:
            progress.progress(
                ...
                text=f"Processing recorded video... {frame_no}/{total_frames} frames",
            )
...
with tab2:
    section_header(
        ...
        "Upload a recorded video, process frames in batch, generate an annotated result video, and review the video by downloading it",
    )
    st.markdown(
        "<div class='tip-box'><b>Lower the frames</b> the more time will be taken for video processing.</div>",
        unsafe_allow_html=True,
    )
    ...
    if not db_video:
        ...
        with rvc1:
            recorded_threshold = st.slider(
                "Recorded video threshold",
                0.1,
                1.0,
                threshold,
                0.05,
                key="recorded_video_thresh",
            )
        with rvc2:
            frames_stride = st.selectbox(
                "Process every Nth frame",
                options=[1, 2, 3, 5],
                index=0,
                help="Higher values are faster but may miss brief appearances.",
            )
        with rvc3:
            st.caption(
                f"Database size: {len(db_video)} people. Lower stride is slower but more thorough."
            )

        recorded_video_file = st.file_uploader(
            "Upload Recorded Video",
            type=["mp4", "avi", "mov", "mkv"],
            key="recorded_video_uploader",
        )

        if recorded_video_file is not None:
            # Preview uses getvalue() which does not move the pointer
            st.video(recorded_video_file.getvalue())

        if st.button(
            "Run Detection on Recorded Video",
            disabled=recorded_video_file is None,
            key="run_recorded_video_detection",
        ):
            if recorded_video_file is None:
                st.error("Please upload a recorded video first.")
            else:
                ...
                with tempfile.NamedTemporaryFile(delete=False, suffix=input_suffix) as tmp_in:
                    recorded_video_file.seek(0)  # critical fix: reset pointer before read
                ...
                with st.spinner("Processing recorded video for face detection..."):
                    summary, err = process_recorded_video(
                        ...
                        frame_stride=int(frames_stride),
                    )
                ...
                if err:
                    ...
                    add_surveillance_entry(
                        ...
                        known=summary["known_faces_total"],
                        unknown=summary["unknown_faces_total"],
        ...
        if "processed_recorded_video_bytes" in st.session_state:
            ...
            s4.metric("Known Faces Frames", summary.get("known_faces_total", 0))
            # s5.metric("Unknown Faces Frames", summary.get("unknown_faces_total", 0))

            if summary.get("unknown_faces_total", 0) > 0:
                status_banner(
                    "danger",
                    "Review recommended",
                    "The processed video contains one or more unknown-face detections.",
                )
            else:
                status_banner(
                    "success",
                    "Processing clear",
                    "No unknown faces were detected in the processed output video.",
                )
